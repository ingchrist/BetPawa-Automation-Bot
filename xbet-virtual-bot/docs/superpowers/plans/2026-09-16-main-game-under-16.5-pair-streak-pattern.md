...
- Before anything else: open the browser's My Bets and confirm the placed selection reads exactly `Total. Main game Under 16.5`, not a half-scoped market (`Total. 1st half ...` or `Total. 2nd half ...`) — `betting_api.py`'s own module comment documents, from real-money A/B testing, that betting the raw `match_id` is ambiguous specifically in the pregame window, and Pattern 4 (unlike Patterns 1-3) can fire and arm pregame. If it lands as a half market instead of Main game, that's a real bug — the fix is to bet `period=0` only once the match is live, not before.
